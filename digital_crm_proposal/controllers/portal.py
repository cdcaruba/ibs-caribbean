...
class CustomerPortal(CustomerPortal):
    ...
    def portal_order_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        if report_type in ('html', 'pdf', 'text'):
            ...
            if order_details and order_details.is_proposal:
                return self._show_report(model=order_sudo, report_type=report_type, report_ref='digital_crm_proposal.action_report_proposal', download=download)
            else:
    ...
    def portal_quote_accept(self, order_id, access_token=None, name=None, signature=None):
        ...
        order_details =  request.env['sale.order'].sudo().search([('id','=',order_id)], limit=1)
        if order_details and order_details.is_proposal:
            pdf = request.env.ref('digital_crm_proposal.action_report_proposal').sudo().render_qweb_pdf([order_sudo.id])[0]
            # return self._show_report(model=order_sudo, report_type=report_type, report_ref='digital_crm_proposal.action_report_proposal', download=download)
